...
def control_pwm1():
    ...
    while control_pwm_enabled:
        ...
        if actual_speed1 > 0:
            ...
            time.sleep(motor_pwm_period * actual_speed1)

            GPIO.output(15, GPIO.LOW)
            time.sleep(motor_pwm_period * (1 - actual_speed1))
        else:
...
def control_pwm2():
    ...
    while control_pwm_enabled:
        if actual_speed2 > 0:
            ...
            time.sleep(motor_pwm_period * actual_speed2)

            GPIO.output(19, GPIO.LOW)
            time.sleep(motor_pwm_period * (1 - actual_speed2))
        else:
            time.sleep(motor_pwm_period)
...
def control_pwm3():
    ...
    while control_pwm_enabled:
        if actual_speed3 > 0:
            ...
            time.sleep(motor_pwm_period * actual_speed3)

            GPIO.output(11, GPIO.LOW)
            time.sleep(max(motor_pwm_period * (1 - actual_speed3))
        else:
            time.sleep(motor_pwm_period)
...
def control_pwm4():
    ...
    while control_pwm_enabled:
        ...
        if actual_speed4 > 0:
            ...
            time.sleep(motor_pwm_period * actual_speed4)

            GPIO.output(13, GPIO.LOW)
            time.sleep(motor_pwm_period * (1 - actual_speed4))
        else:
            time.sleep(motor_pwm_period)
'''
def hc_sensor():
    ...
    while drive:
        ...
        for event in gamepad.read_loop():
            ...
            if event.type == ecodes.EV_ABS:
                    normalized = event.value - 128
                if event.code == ecodes.ABS_Y:
                    actual_speed1 = max(0, -normalized/128)
                    actual_speed2 = max(0, normalized/128)
                    actual_speed3 = max(0, normalized/128)
                    actual_speed4 = max(0, -normalized/128)
                if event.code == ecodes.ABS_Z:
                    if normalized < 0:
                        if w == 1: # rotate movement
                            actual_speed2 = 0
                        else:
                            actual_speed2 = max(0, -normalized/128)
                        actual_speed4 = max(0, -normalized/128)
